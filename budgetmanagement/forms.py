...
def get_tranche_form(slug):
	...
	class TrancheForm(forms.ModelForm):
		...
		recommended_by = forms.ModelChoiceField(queryset=UserProfile.objects.filter(id__in=[i.id for i in tt]),required=True, widget = forms.Select(attrs={'class': 'form-control'}))
